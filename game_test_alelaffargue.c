...
bool test_game_set_square() {
  ...
  if (game_get_square(g, 1, 0) != S_ONE ||
      game_get_square(g, 0, 1) != S_IMMUTABLE_ONE) {
    game_delete(g);
    return false;
  }
...
bool test_game_get_square() {
  ...
  free(square_array);
  if (game_get_square(g, 0, 0) != S_ZERO || game_get_square(g, 0, 1) != S_ONE ||
      game_get_square(g, 0, 2) != S_IMMUTABLE_ONE ||
      game_get_square(g, 0, 3) != S_IMMUTABLE_ZERO ||
...
bool test_game_get_number() {
  ...
  if (game_get_number(g, 0, 0) != 0 || game_get_number(g, 0, 1) != 1 ||
      game_get_number(g, 0, 2) != 1 || game_get_number(g, 0, 3) != 0 ||
...
bool test_game_get_next_square() {
  ...
  game g = game_new(square_array);
  free(square_array);
  // test à l'interieur de la grille
  if (game_get_next_square(g, 1, 0, UP, 1) != S_ZERO ||
      game_get_next_square(g, 1, 0, DOWN, 1) != S_ONE ||
      game_get_next_square(g, 0, 1, LEFT, 1) != S_ZERO ||
      game_get_next_square(g, 0, 1, RIGHT, 1) != S_IMMUTABLE_ONE ||
      game_get_next_square(g, 2, 0, UP, 2) != S_ZERO ||
      game_get_next_square(g, 0, 0, DOWN, 2) != S_ONE ||
      game_get_next_square(g, 0, 5, LEFT, 2) != S_IMMUTABLE_ZERO ||
      game_get_next_square(g, 2, 0, RIGHT, 2) != S_EMPTY) {
    game_delete(g);
    return false;
  }
  // début test à travers les bordures avec l'option wrapping fausse.
  else if (game_get_next_square(g, 0, 5, RIGHT, 1) != -1 ||
           game_get_next_square(g, 0, 5, RIGHT, 2) != -1 ||
           game_get_next_square(g, 1, 0, LEFT, 1) != -1 ||
           game_get_next_square(g, 3, 1, LEFT, 2) != -1 ||
           game_get_next_square(g, 0, 2, UP, 1) != -1 ||
           game_get_next_square(g, 1, 4, UP, 2) != -1 ||
           game_get_next_square(g, 5, 1, DOWN, 1) != -1 ||
           game_get_next_square(g, 4, 2, DOWN, 2) != -1) {
    game_delete(g);
    return false;
  }
  game_delete(g);
  ...
  if (game_get_next_square(g_with_wrapping, 2, 5, RIGHT, 1) != S_ZERO ||
      game_get_next_square(g_with_wrapping, 1, 5, RIGHT, 2) != S_ONE ||
      game_get_next_square(g_with_wrapping, 3, 0, LEFT, 1) !=
          S_IMMUTABLE_ZERO ||
      game_get_next_square(g_with_wrapping, 2, 1, LEFT, 2) != S_IMMUTABLE_ONE ||
      game_get_next_square(g_with_wrapping, 0, 2, UP, 1) != S_ONE ||
      game_get_next_square(g_with_wrapping, 1, 4, UP, 2) != S_ZERO ||
      game_get_next_square(g_with_wrapping, 3, 3, DOWN, 1) != S_ONE ||
      game_get_next_square(g_with_wrapping, 3, 2, DOWN, 2) != S_IMMUTABLE_ONE) {
...
bool test_game_get_next_number() {
  ...
  if (game_get_next_number(g, 1, 0, UP, 1) != 0 ||
      game_get_next_number(g, 1, 0, DOWN, 1) != 1 ||
      game_get_next_number(g, 0, 1, LEFT, 1) != 0 ||
      game_get_next_number(g, 0, 1, RIGHT, 1) != 1 ||
      game_get_next_number(g, 2, 0, UP, 2) != 0 ||
      game_get_next_number(g, 0, 0, DOWN, 2) != 1 ||
      game_get_next_number(g, 0, 5, LEFT, 2) != 0 ||
      game_get_next_number(g, 2, 0, RIGHT, 2) != -1) {
    game_delete(g);
    return false;
  }
  // début test à travers les bordures avec l'option wrapping fausse.
  else if (game_get_next_number(g, 0, 5, RIGHT, 1) != -1 ||
           game_get_next_number(g, 0, 5, RIGHT, 2) != -1 ||
           game_get_next_number(g, 1, 0, LEFT, 1) != -1 ||
           game_get_next_number(g, 3, 1, LEFT, 2) != -1 ||
           game_get_next_number(g, 0, 2, UP, 1) != -1 ||
           game_get_next_number(g, 1, 4, UP, 2) != -1 ||
           game_get_next_number(g, 5, 1, DOWN, 1) != -1 ||
           game_get_next_number(g, 4, 2, DOWN, 2) != -1) {
    game_delete(g);
    return false;
  }
  game_delete(g);
  ...
  free(square_array_with_wrapping);
  // début test à travers les bordures
  if (game_get_next_number(g_with_wrapping, 2, 5, RIGHT, 1) != 0 ||
      game_get_next_number(g_with_wrapping, 1, 5, RIGHT, 2) != 1 ||
      game_get_next_number(g_with_wrapping, 3, 0, LEFT, 1) != 0 ||
      game_get_next_number(g_with_wrapping, 2, 1, LEFT, 2) != 1 ||
      game_get_next_number(g_with_wrapping, 0, 2, UP, 1) != 1 ||
      game_get_next_number(g_with_wrapping, 1, 4, UP, 2) != 0 ||
      game_get_next_number(g_with_wrapping, 3, 3, DOWN, 1) != 1 ||
      game_get_next_number(g_with_wrapping, 3, 2, DOWN, 2) != 1) {
    game_delete(g_with_wrapping);
    return false;
  }
  game_delete(g_with_wrapping);
...
bool test_game_is_immutable() {
  ...
  game g = game_new(square_array);
  free(square_array);
  if (!game_is_immutable(g, 0, 2) || !game_is_immutable(g, 0, 3) ||
      !game_is_immutable(g, 1, 0) || game_is_immutable(g, 0, 0) ||
      game_is_immutable(g, 0, 1) || game_is_immutable(g, 0, 4)) {
    game_delete(g);
    return false;
  }
  game_delete(g);
